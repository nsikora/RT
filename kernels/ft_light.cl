...
__kernel void				ft_raytracer_light(const __private t_dec	ambient,
											...
											__global t_ray				*rays)
{
	...
	if (ray.id >= 0)
	{
		...
		else if (objs[ray.id].texture == -3)
			color = ft_texture_perlin(&ray, objs + ray.id);
		else
			color = img[id];
		c = ft_color_bright(color, ambient);
   		ft_light(&c, ray, lights, objs + ray.id, objs, textures, color);
		img[id] = c;
	}
}
