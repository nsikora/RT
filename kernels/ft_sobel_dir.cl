...
static int	apply_sobel(t_color pix[3][3], const int g_x[3][3], const int g_y[3][3], __global float *dir);

void ft_get_small_pix_mat(const int *img,
							const int pos,
							const int width,
							t_color pix[3][3])
{
...
static int	apply_sobel(t_color pix[3][3], const int g_x[3][3],
						const int g_y[3][3], __global float *dir)
{
	int		i;
	int		j;
	float	result[3];
...
__kernel void				ft_rt_sobel_dir(const __private int			width,
										...
										__global float				*dir)
{
	...
	else
	{
		...
		dest[id].rgb = apply_sobel(pix, g_x, g_y, &dir[id]);
	}
}
